...
app.controller("myContro", function ($scope) {
    ...
    function GetAndShowData() {
        $.ajax({
            cache: true,//保留缓存数据
            type: "get",//为post请求
            url: "../User/SelectUserListMeth",//这是我在后台接受数据的文件名
            data: "pagIndex=" + pagIndex + "&nickname=" + $scope.condition.nickname + "&subscribe=" + $scope.condition.subscribe,//将该表单序列化
