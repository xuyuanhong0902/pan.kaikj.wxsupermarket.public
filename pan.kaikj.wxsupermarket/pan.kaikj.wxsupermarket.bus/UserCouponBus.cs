...
namespace pan.kaikj.wxsupermarket.bus
{
    ...
    public class UserCouponBus
    {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public string UseCoupon(string id, string orderId)
        {
            MwxResult mwxResult = new MwxResult()
            {
                errcode = -1
            ...
            try
            {
                if (new UserCouponService().UseCoupon(id, orderId) > 0)
                {
                    mwxResult.errcode = 0;
                    mwxResult.errmsg = "操作成功！";
                }
        ...
        public string Delete(string id)
        {
            ...
            try
            {
                ...
                {
                    if (!new UserCouponService().Delete(id))
                    {
                        mwxResult.errmsg = "操作失败";
                    }
                    else
